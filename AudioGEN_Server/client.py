...
def generate_and_save(task_id: str, scenes: list):
    ...
    try:
        resp = requests.post(f"{BASE_URL}/generate/audios", json=payload, timeout=60)
        resp.raise_for_status()
        data = resp.json()
        return_task_id = data["task_id"]
        audio_paths = data["audio_paths"]
        print(f"success! returned task id: {return_task_id}")
        print(f"total {len(audio_paths)} images")
        return audio_paths
...
def download_audios(task_id: str, audios_paths: List[str]):
    task_dir = LOCAL_TASKS_DIR / task_id
    task_dir.mkdir(parents=True, exist_ok=True)

    for filename in audios_paths:
...
if __name__ == "__main__":
    ...
    return_audio_paths = generate_and_save(MOCK_TASK_ID, MOCK_STORYBOARD_JSON["scenes"])
    # print(return_audio_paths)
    download_audios(MOCK_TASK_ID, return_audio_paths)